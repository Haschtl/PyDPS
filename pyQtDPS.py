...
class PyQtDPS(QtWidgets.QMainWindow):
    ...
    def setDisplay(self, a):
        self.ifValLCD(a[2]/100, self.voltageLCD)
        self.ifValLCD(a[3]/100, self.ampLCD)
        self.ifValLCD(a[4]/100, self.powerLCD)
        if self.vInLabel.text() != str(a[5]/100)+" V (V-In)":
            self.vInLabel.setText(str(a[5]/100)+" V (V-In)")
        if self.voltageSpinBox.maximum() != round(a[5]/100-1,2):
            self.voltageSpinBox.setMaximum(round(a[5]/100-1,2))
        if(a[9]==1):
            self.powerButton.setStyleSheet("background-color: green")
        else:
            self.powerButton.setStyleSheet("background-color: red")
        if a[0]/100==round(self.voltageSpinBox.value(),2):
            self.voltageSpinBox.setStyleSheet("background-color: green")
        else:
            self.voltageSpinBox.setStyleSheet("background-color: red")
        if a[1]/100==round(self.ampSpinBox.value(),2):
    ...
    def plotValues(self):
        a=self.power_supply.read_registers(0,11)
        now = time.time()
        self.plotTime.append(float(now))
        self.plotVolt.append(a[2]/100)
        self.plotAmp.append(a[3]/100)
        self.plotPower.append(a[4]/100)
